...
document.getElementById('pemasukanHapusBtn').addEventListener('click', async function() {
    ...
    // Step 2: Hapus dari Dexie + Firestore
    try {
        ...
        if (existing) {
            ...
            // PENTING: Juga hapus dari Firestore supaya tidak muncul lagi saat reload
            if (firestoreSiap) {
                await fsdb.collection('cashIncome').doc(String(existing.id)).delete();
                console.log('[DELETE OK] bulan='+cashBulan+' deleted dari server');
            }
        }
    } catch(err) {
        console.error('Delete error:', err);
        showToast('⚠️ Gagal hapus dari server: '+err.message, 'warning');
        return;
    }
    ...
    showToast('✅ Data telah dihapus dari server.','success');
});
